...
// .wrangler/tmp/bundle-cn4v0r/strip-cf-connecting-ip-header.js
...
var HonoRequest = /* @__PURE__ */ __name(class {
   ...
   */
  get matchedRoutes() {
    return this.#matchResult[0].map(([[, route8]]) => route8);
  }
  /**
   ...
   */
  get routePath() {
    return this.#matchResult[0].map(([[, route8]]) => route8)[this.routeIndex].path;
  }
}, "HonoRequest");
...
// src/routes/check-db.js
var route = new Hono2();
route.get("/check-db", async (c) => {
  try {
    const db = getDb(c.env);
    const { results: tables } = await db.prepare("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name").all();
    return c.json({
      ok: true,
...
function getSecret(env2) {
  const raw2 = env2 && env2.JWT_SECRET ? env2.JWT_SECRET : "pfm_dev_secret_change_me";
  return new TextEncoder().encode(raw2);
}
__name(getSecret, "getSecret");
async function signStaffToken(staffUser, env2) {
  const allowedRooms = Array.isArray(staffUser.allowed_rooms) ? staffUser.allowed_rooms : JSON.parse(staffUser.allowed_rooms || "[]");
  return new SignJWT({
  ...
  }).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime("8h").sign(getSecret(env2));
}
__name(signStaffToken, "signStaffToken");
...
function requireRoomAccess(roomField = "room_id") {
  return async (c, next) => {
    const auth = c.get("auth");
    if (!auth)
...
__name(requireRoomAccess, "requireRoomAccess");

// src/routes/check-auth.js
var route2 = new Hono2();
route2.get("/check-auth", authMiddleware, async (c) => {
  const auth = c.get("auth");
  const db = getDb(c.env);
...
var check_auth_default = route2;

// src/routes/auth.js
var route3 = new Hono2();
route3.get("/auth/users", async (c) => {
  const db = getDb(c.env);
  const { results: staffUsers } = await db.prepare("SELECT id, username, full_name, role, allowed_rooms FROM staff_users WHERE is_active = 1 ORDER BY username ASC").all();
  return c.json({
    ok: true,
...
route3.post("/auth/login", async (c) => {
  const db = getDb(c.env);
  ...
  const staff = await db.prepare("SELECT * FROM staff_users WHERE username = ? AND is_active = 1 LIMIT 1").bind(username).first();
  if (!staff) {
...
route3.post("/auth/refresh-token", async (c) => {
  const db = getDb(c.env);
  const body = await c.req.json().catch(() => ({}));
  ...
  try {
    ...
    const staff = await db.prepare("SELECT * FROM staff_users WHERE id = ? AND is_active = 1 LIMIT 1").bind(staffId).first();
    if (!staff) {
      return c.json({ ok: false, error: "staff_not_found" }, 404);
    }
    const parsedStaff = { ...staff, allowed_rooms: JSON.parse(staff.allowed_rooms || "[]") };
...
var auth_default = route3;

// src/routes/modules.js
var route4 = new Hono2();
route4.get("/v1/modules/floor-room", authMiddleware, async (c) => {
  const db = getDb(c.env);
  const { results: floors } = await db.prepare("SELECT * FROM floors WHERE is_active = 1 ORDER BY sort_order ASC").all();
  const { results: rooms } = await db.prepare("SELECT * FROM rooms WHERE is_active = 1 ORDER BY sort_order ASC").all();
  const floorMap = floors.map((floor) => ({
    floor_id: floor.floor_id,
    floor_name: floor.floor_name,
    sort_order: floor.sort_order,
    rooms: rooms.filter((room) => room.floor_id === floor.floor_id).map((room) => ({
...
var ACTIVE_QUEUE_STATUSES = [
  QUEUE_STATUS.CHO_KHAM,
  QUEUE_STATUS.DANG_KHAM,
  QUEUE_STATUS.CHO_KET_QUA,
  QUEUE_STATUS.CHO_TAI_KHAM
];
var WAITING_QUEUE_STATUSES = [
  QUEUE_STATUS.CHO_KHAM,
  QUEUE_STATUS.CHO_TAI_KHAM
];

// src/lib/queue-service.js
var HO_CHI_MINH_TIMEZONE = "Asia/Ho_Chi_Minh";
...
function parsePatientPayload(raw2 = {}) {
  const medicalCode = normalizeText(
    raw2.medical_code ?? raw2.medicalCode ?? raw2.mayte ?? raw2.ma_y_te ?? ""
  );
  const identityNumber = normalizeText(
    raw2.identity_number ?? raw2.identityNumber ?? raw2.socmt ?? raw2.cccd ?? raw2.cmnd ?? ""
  );
  return {
    medical_code: medicalCode,
    identity_number: identityNumber,
    patient_key: medicalCode && identityNumber ? `${medicalCode}|${identityNumber}` : "",
    full_name: String(raw2.full_name ?? raw2.fullName ?? raw2.hoten ?? raw2.name ?? "").trim(),
    dob: String(raw2.dob ?? raw2.namsinh ?? raw2.ngaysinh ?? "").trim(),
    gender: String(raw2.gender ?? raw2.gioitinh ?? "").trim(),
    address: String(raw2.address ?? raw2.diachi ?? "").trim(),
    address_cv30: String(raw2.address_cv30 ?? raw2.diachi_cv30 ?? "").trim(),
    is_priority: toBoolean(raw2.is_priority ?? raw2.isPriority ?? false),
    is_online_booking: toBoolean(raw2.is_online_booking ?? raw2.isOnlineBooking ?? false),
    source_payload: raw2
  };
}
__name(parsePatientPayload, "parsePatientPayload");
function parseQrContent(raw2) {
...
__name(parseQrContent, "parseQrContent");
function buildPatientQrPayload(patientDoc) {
  return {
...
__name(buildPatientQrPayload, "buildPatientQrPayload");
async function ensureRoomExists(db, roomId) {
  const room = await db.prepare("SELECT * FROM rooms WHERE room_id = ? AND is_active = 1 LIMIT 1").bind(roomId).first();
  if (!room)
    throw new Error(`room_not_found:${roomId}`);
  return room;
}
__name(ensureRoomExists, "ensureRoomExists");
async function nextQueueNumber(db, roomId) {
...
__name(nextQueueNumber, "nextQueueNumber");
async function appendQueueEvent(db, event) {
  const now = Date.now();
  await db.prepare(
...
__name(appendQueueEvent, "appendQueueEvent");
async function nextOrderRank(db, roomId, isPriority) {
  ...
  const { results } = await db.prepare(`SELECT order_rank FROM queues WHERE room_id = ? AND status IN (${ph}) AND priority_rank = ? ORDER BY order_rank ASC`).bind(roomId, ...WAITING_QUEUE_STATUSES, priorityRank).all();
  if (results.length === 0)
    return 1;
...
__name(nextOrderRank, "nextOrderRank");
...
async function findQueueById(db, queueId) {
...
__name(findQueueById, "findQueueById");
async function findLatestActiveQueueForPatientRoom(db, patientId, roomId) {
  const ph = ACTIVE_QUEUE_STATUSES.map(() => "?").join(",");
  return db.prepare(`SELECT * FROM queues WHERE patient_id = ? AND room_id = ? AND status IN (${ph}) ORDER BY created_at DESC, order_rank DESC LIMIT 1`).bind(patientId, roomId, ...ACTIVE_QUEUE_STATUSES).first();
}
__name(findLatestActiveQueueForPatientRoom, "findLatestActiveQueueForPatientRoom");
async function findPatientById(db, patientId) {
  if (!isValidUUID(patientId))
    return null;
  return db.prepare("SELECT * FROM patients WHERE id = ? LIMIT 1").bind(patientId).first();
}
__name(findPatientById, "findPatientById");
async function upsertPatient(db, patientData) {
  const now = Date.now();
  ...
  const sourcePayloadJson = patientData.source_payload ? JSON.stringify(patientData.source_payload) : null;
...
__name(upsertPatient, "upsertPatient");
async function updateQueueStatus(db, {
  queue,
  status,
  actorUserId,
  note,
  ...
  await db.prepare(
    `UPDATE queues SET status = ?,
        ...
        updated_at   = ?
       WHERE id = ?`
  ).bind(status, calledAt, calledAt, completedAt, completedAt, now, queue.id).run();
  await appendQueueEvent(db, {
...
__name(updateQueueStatus, "updateQueueStatus");
async function getRoomQueues(db, roomId, statuses = ACTIVE_QUEUE_STATUSES, date = null) {
...
__name(getRoomQueues, "getRoomQueues");
async function moveQueuePosition(db, {
  queueId,
  roomId,
  targetQueueId = null,
  mode = "before",
  ...
  const ph = WAITING_QUEUE_STATUSES.map(() => "?").join(",");
  const { results: docs } = await db.prepare(`SELECT * FROM queues WHERE room_id = ? AND status IN (${ph}) ORDER BY priority_rank ASC, order_rank ASC, created_at ASC`).bind(roomId, ...WAITING_QUEUE_STATUSES).all();
  ...
  if (Array.isArray(orderedIds) && orderedIds.length > 0) {
    ...
    const now2 = Date.now();
    const updates2 = requestedIds.map(
      (id, i) => db.prepare("UPDATE queues SET order_rank = ?, updated_at = ? WHERE id = ?").bind(i + 1, now2, id)
    );
    await db.batch(updates2);
    return { updated: updates2.length, ordered_ids: requestedIds };
  }
  ...
  const updates = docs.map(
    (q, i) => db.prepare("UPDATE queues SET order_rank = ?, updated_at = ? WHERE id = ?").bind(i + 1, now, q.id)
  );
  await db.batch(updates);
  return { updated: updates.length, ordered_ids: docs.map((item) => item.id) };
}
__name(moveQueuePosition, "moveQueuePosition");
async function toQueueView(db, queueDoc) {
  const patient = await db.prepare("SELECT * FROM patients WHERE id = ? LIMIT 1").bind(queueDoc.patient_id).first();
...
__name(toQueueView, "toQueueView");
function parseQueueId(id) {
  if (!isValidUUID(String(id || "")))
    throw new Error("invalid_queue_id");
  return id;
}
__name(parseQueueId, "parseQueueId");
...
__name(serializePatient, "serializePatient");
route5.post(
  "/v1/patients",
  authMiddleware,
...
route5.get("/v1/patients/:id", authMiddleware, async (c) => {
  const db = getDb(c.env);
...
route5.get("/v1/patients/:id/qr", authMiddleware, async (c) => {
  const db = getDb(c.env);
  const id = c.req.param("id");
  if (!UUID_RE.test(id)) {
    return c.json({ ok: false, error: "invalid_patient_id" }, 400);
...
var patients_default = route5;

// src/lib/queue-events.js
async function emitQueueUpdate(env2, roomId, roomName, items) {
  const payload = JSON.stringify({ roomId, roomName: roomName || roomId, items });
  try {
    const id = env2.ROOM_HUB.idFromName(roomId);
    const stub = env2.ROOM_HUB.get(id);
  ...
  try {
    const pubId = env2.ROOM_HUB.idFromName(`public:${roomId}`);
    const pubStub = env2.ROOM_HUB.get(pubId);
...
__name(emitQueueUpdate, "emitQueueUpdate");

// src/routes/queue.js
var route6 = new Hono2();
async function broadcastRoom(env2, roomId) {
  try {
    const db = getDb(env2);
    ...
    await emitQueueUpdate(env2, roomId, roomName, items);
  } catch {
  }
}
__name(broadcastRoom, "broadcastRoom");
...
__name(ensureQueueRoomAccessOrThrow, "ensureQueueRoomAccessOrThrow");
route6.get(
  "/v1/queue",
  authMiddleware,
  ...
  async (c) => {
    ...
    const statuses = statusRaw ? statusRaw.split(",").map((item) => item.trim()).filter((item) => item && isValidQueueStatus(item)) : ACTIVE_QUEUE_STATUSES;
...
route6.patch(
  ...
  async (c) => {
    ...
    try {
      ...
      const statusCode = message2 === "queue_not_found" ? 404 : message2 === "queue_room_mismatch" ? 400 : message2 === "queue_not_movable" ? 409 : message2 === "queue_not_in_waiting_list" ? 409 : message2 === "target_queue_not_found" ? 404 : message2 === "invalid_position_mode" ? 400 : message2 === "ordered_ids_mismatch" ? 400 : 400;
      return c.json({ ok: false, error: message2 || "position_update_failed" }, statusCode);
    }
    const items = [];
...
var queue_default = route6;

// src/routes/events.js
var route7 = new Hono2();
route7.get("/v1/ws/queue", async (c) => {
...
route7.get("/v1/ws/public", async (c) => {
  const roomId = String(c.req.query("room_id") || "").trim();
  if (!roomId)
    return c.json({ ok: false, error: "room_id_required" }, 400);
  const dateParam = String(c.req.query("date") || "").trim();
...
__name(toPublicViewFromView, "toPublicViewFromView");
var events_default = route7;

// src/durable-objects/RoomHub.js
var RoomHub = class {
...
app.route("/api", check_db_default);
app.route("/api", check_auth_default);
app.route("/api", auth_default);
app.route("/api", modules_default);
app.route("/api", patients_default);
app.route("/api", queue_default);
app.route("/api", events_default);
var src_default = app;
...
// .wrangler/tmp/bundle-cn4v0r/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-cn4v0r/middleware-loader.entry.ts
